# KNOU-CS AI Navigator

한국방송통신대학교 컴퓨터과학과 홈페이지의 공개 정보를 수집해 Notion 지식 DB에 적재하고, 공식 데이터 검색을 우선하는 RAG 챗봇 **컴누리**를 제공하는 경진대회 출품 프로젝트입니다.
...
본 프로젝트는 인공지능 강의에서 학습한 개념을 실제 서비스로 구현한 사례이다.

- **논리에 의한 지식표현:**  
  사용자의 질문을 키워드와 조건으로 분해하고, Notion DB에 저장된 구조화된 지식과 매칭한다.

- **퍼지이론:**  
  사용자의 모호한 질문도 키워드 유사도와 점수 기반으로 가장 가까운 공식 정보를 탐색한다.

- **머신러닝/인공신경망:**  
  질문과 문서를 벡터화하여 유사도를 계산하고, 가장 관련성 높은 정보를 선택한다. 현재 기본 구현은 배포 안정성을 위해 경량 토큰 벡터와 TF-IDF 성격의 역문서 빈도 점수를 사용하며, 향후 FAISS 임베딩 인덱스로 교체할 수 있다.

- **딥러닝/Transformer:**  
  LLM은 검색 결과가 없을 때만 제한적으로 사용하며, 문맥 기반 자연어 응답을 생성한다.

- **RAG 구조:**  
  LLM이 임의로 답변하지 않도록 공식 DB 검색 결과를 우선 사용하는 검색 증강 생성 구조를 적용한다.
